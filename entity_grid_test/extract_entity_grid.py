    ...
    if target_id <= index and tokens[0].startswith("(NP"):
      for token in tokens:
        ...
        if token.startswith("NN"):
          ...
          nouns.append(word.lower(
          ))  # is noun phrase, only one DT word (this, all) in the phrase
      break
...
for filename in os.listdir("text/"):
  print(filename)
  with open("text/" + filename, 'r') as in_file:
    ...
    const_out = open("parsed/" + text_id + ".const_parse", "w")
    dep_out = open("parsed/" + text_id + ".dep_parse", "w")
    grid_out = open("grid/" + text_id + ".txt", "w")
    # read text document
    document_lines = []
